...
namespace uTrans.Components
{
   ...
   */
    protected Vector3[] endPoints
    {
      get
      {
        Vector3 pos = T.position, ray = T.rotation * (stretchAxis * T.lossyScale.z / 2);   // Half the stretch (T.forward also works here)
   ...
   */
    protected virtual void Update()
    {
      ...
      for (int i = 0; i < 2; i++)
      {
        ...
        if (oldTargetPoint[i] != tlp)
        {
          // Take note of any change in position
      ...
      float localDistance = targetDiff.magnitude, localMargin0 = targetMargin[0] / scaleFactor, localMargin1 = targetMargin[1] / scaleFactor, lengthScale = localDistance - (localMargin0 + localMargin1);
